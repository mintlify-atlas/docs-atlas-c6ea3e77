...
After registering, you are redirected to `/users/profile/` where you should complete your company profile straight away. Students and companies both see a **Company** badge (amber) in the navbar so the active role is always visible.
...
<Steps>
  ...
  <Step title="View a student's public profile">
    Click on a student's name to open their public profile at `/users/profile/<id>/`. The public profile page displays the student's full name, university, degree, bio, skills, and any CV they have uploaded — all the information you need to make an informed decision.
  </Step>
  <Step title="Accept or reject the application">
    Make your decision by sending a `POST` request to the appropriate URL:

    ```
    POST /applications/<id>/accepted/   →  status set to "accepted"
    POST /applications/<id>/rejected/   →  status set to "rejected"
    ```
...
Keep your company profile up to date so students have accurate context when browsing your listings. Navigate to `/users/profile/edit/` to update the following fields:
