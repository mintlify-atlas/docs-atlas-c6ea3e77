...
<Steps>
  ...
  <Step title="Install the PostgreSQL adapter">
    Add `psycopg2-binary` to your `requirements.txt` (or install it directly):
...
<Warning>
  Never commit database credentials directly to source control. Use environment variables or a tool like `django-environ` to load secrets from a `.env` file, and add that file to `.gitignore`.
</Warning>
